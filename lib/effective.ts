...
/**
 * Effective main pick for every category on one lifestyle.
...
export function effectivePicks(catalog: Catalog, tierId: string): Map<string, EffectivePick> {
  ...
  for (const pick of catalog.picks) {
    picks.set(pickKey(pick.tier, pick.category), pick);
  }
  ...
  for (const category of catalog.categories) {
    result.set(category.id, resolveCategory(tierId, category.id, picks, basedOnOf, new Set()));
  }
  return result;
}
